...
(function () {
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const response = await fetch(`http://192.168.0.200:3000/api/ordenes${id ? `/${id}/solicitud` : ''}`, { method: id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('token')}` }, body: JSON.stringify(payload) });
